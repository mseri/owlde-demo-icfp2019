...
let () =
  ...
  Command.run (Bench.make_command [
    Bench.Test.create ~name:"RK45" 
      (fun () -> Ode.odeint custom_rk45 f y0 tspec ());
    Bench.Test.create ~name:"RK4" 
      (fun () -> Ode.odeint Native.D.rk4 f y0 tspec ());
    Bench.Test.create ~name:"LSODA" 
      (fun () -> Ode.odeint (Owl_ode_odepack.lsoda ~relative_tol:1E-3 ~abs_tol:1E-6) f y0 tspec ())
  ])
